...
#Mapas de tempo relativo por distrito - origens e destinos ---------------
...
for (i in 1:length(mapsp@data$ds_codigo)){
  ...
  aux = viagenspares[viagenspares$ID %in% oriaux$`coordsor$ID`,]
  ...
  #GWR "Univariado"  
  gwr.res <- gwr.basic(temprelativo~temprelativo,
                       data=spatialaux,
  ...
  #gravacao do mapa
  jpeg(paste("distritos/GWR_Origem_Tempo_Relativo_escala_uniforme_", tipo,"_",distrito,".jpg", sep = ""), width = 700, height = 500)
  #jpeg(paste("distritos/GWR_Origin_Time_Relative_scale_uniform_", tipo,"_",distrito,".jpg", sep = ""), width = 700, height = 500)
   layout(mat = 1,widths = 1.4, heights = 2)
  plot(gwr.res$SDF[mapsp,], col = rev(heat.colors(50)), zlim = c(1,5))
  title(main = titulo)
  #title(main = "Travel times Ratio - District of Jabaquara")
...
#Mapas de tempo de viagem publico por distrito - origens e destinos ---------------
...
for (i in 1:length(mapsp@data$ds_codigo)){
  ...
  oriaux=cbind(coordsor$ID, oriaux)
  oriaux=oriaux[is.na(oriaux$V2)==FALSE,]
  aux = viagenspublicas[viagenspublicas$ID %in% oriaux$`coordsor$ID`,]
  spatialaux <- spatialdestinations[spatialdestinations$ID %in% aux$ID,]
  spatialaux@data <- merge(spatialaux@data, aux, by = "ID")
  ...
  DM <- gw.dist(dp.locat=coordinates(spatialaux),rp.locat=coordinates(grd))
  #GWR "Univariado"  
  gwr.res <- gwr.basic(duration~duration,
                       data=spatialaux,
                       regression.points=grd,
                       adaptive = TRUE,
                       bw=bandwidth.gwr,
  ...
  titulo <- paste("Regressão GWR Univ.", tema, subtitulo, sep = " - ")
  
  #gravacao do mapa
  jpeg(paste("distritos/GWR_Origem_Tempo_TPublico_escala_uniforme", tipo,"_", distrito,".jpg", sep = ""), width = 700, height = 500)
  #jpeg(paste("distritos/GWR_Origin_Time_Public_scale_uniform_", tipo,"_",distrito,".jpg", sep = ""), width = 700, height = 500)
  layout(mat = 1,widths = 1.4, heights = 2)
  plot(gwr.res$SDF[mapsp,], col = rev(heat.colors(50)), zlim = c(1,20000))
  title(main = titulo)
  #title(main = "Public travel times - District of Jabaquara")
   legend(-46.25,-23.85, title = "Legenda", inset = 0.05, legend=c("Endereços de Origem", "Endereços de Destino"),
         fill = c("blueviolet", "darkgreen"), text.width = 0.15, text.font = 2, cex = 0.6,
         xjust = 0.5, yjust = 0.5)
  #legend(-46.25,-23.85, title = "Legend", inset = 0.05, legend=c("Origins coordinates", "Destinations coordinates"),
...
for (i in 1:length(mapsp@data$ds_codigo)){
  ...
  oriaux=cbind(coordsor$ID, oriaux)
  oriaux=oriaux[is.na(oriaux$V2)==FALSE,]
  aux = viagensprivadas[viagensprivadas$ID %in% oriaux$`coordsor$ID`,]
  spatialaux <- spatialdestinations[spatialdestinations$ID %in% aux$ID,]
  spatialaux@data <- merge(spatialaux@data, aux, by = "ID")
  ...
  distrito <-  str_to_title(mapsp$ds_nome[i])
  subtitulo <- paste("Viagens partindo de", distrito, sep = " ")
  tema <- "Transporte Privado"
  titulo <- paste("Regressão GWR Univ.", tema, subtitulo, sep = " - ")
  
  #gravacao do mapa
  jpeg(paste("distritos/GWR_Origem_Tempo_TPrivado_escala_uniforme", tipo,"_", distrito,".jpg", sep = ""), width = 700, height = 500)
  #jpeg(paste("distritos/GWR_Origin_Time_Private_scale_uniform_", tipo,"_",distrito,".jpg", sep = ""), width = 700, height = 500)
  layout(mat = 1,widths = 1.4, heights = 2)
  plot(gwr.res$SDF[mapsp,], col = rev(heat.colors(50)), zlim = c(1,20000))
  title(main = titulo)
  #title(main = "Private travel times - District of Jabaquara")
   legend(-46.25,-23.85, title = "Legenda", inset = 0.05, legend=c("Endereços de Origem", "Endereços de Destino"),
         fill = c("blueviolet", "darkgreen"), text.width = 0.15, text.font = 2, cex = 0.6,
         xjust = 0.5, yjust = 0.5)
  #legend(-46.25,-23.85, title = "Legend", inset = 0.05, legend=c("Origins coordinates", "Destinations coordinates"),
...
for (i in 1:length(mapsp@data$ID)){
  ...
  #gravacao do mapa
  jpeg(paste("areas de ponderação//GWR_Origem_Tempo_Relativo_escala_uniforme_", tipo,"_",distrito,".jpg", sep = ""), width = 700, height = 500)
  #jpeg(paste("distritos/GWR_Origin_Time_Relative_scale_uniform_", tipo,"_",distrito,".jpg", sep = ""), width = 700, height = 500)
  layout(mat = 1,widths = 1.4, heights = 2)
  plot(gwr.res$SDF[mapsp,], col = rev(heat.colors(50)), zlim = c(1,5))
  title(main = titulo)
...
mapsp <- readOGR(dsn = "/home/rodolfo/Documentos/Programação/PIBIC 2018-2019/Dados/mapas base/LAYER_AREA_PONDERACAO_2010/DEINFO_AREA_PONDERACAO_2010.shp",layer = "DEINFO_AREA_PONDERACAO_2010")
mapsp@proj4string <- CRS("+init=epsg:5533")
mapsp <- spTransform(mapsp, CRS("+init=epsg:4326"))
#criando variavel auxiliar
new_map <- mapsp
...
for (i in 1:length(mapsp@data$ID)){
  ...
  #gravacao do mapa
  jpeg(paste("areas de ponderação/GWR_Origem_Tempo_TPublico_escala_uniforme", tipo,"_", distrito,".jpg", sep = ""), width = 700, height = 500)
  #jpeg(paste("distritos/GWR_Origin_Time_Public_scale_uniform_", tipo,"_",distrito,".jpg", sep = ""), width = 700, height = 500)
  layout(mat = 1,widths = 1.4, heights = 2)
  plot(gwr.res$SDF[mapsp,], col = rev(heat.colors(50)), zlim = c(1,20000))
  title(main = titulo)
...
for (i in 1:length(mapsp@data$ID)){
  ...
  #gravacao do mapa
  jpeg(paste("areas de ponderação/GWR_Origem_Tempo_TPrivado_escala_uniforme", tipo,"_", distrito,".jpg", sep = ""), width = 700, height = 500)
  #jpeg(paste("distritos/GWR_Origin_Time_Private_scale_uniform_", tipo,"_",distrito,".jpg", sep = ""), width = 700, height = 500)
  layout(mat = 1,widths = 1.4, heights = 2)
  plot(gwr.res$SDF[mapsp,], col = rev(heat.colors(50)), zlim = c(1,20000))
  title(main = titulo)
